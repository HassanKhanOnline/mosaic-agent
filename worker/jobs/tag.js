...
import { MODEL, canTag, embed, searchContent, tagImage } from '../lib/tagging';
...
export async function tagTick(env) {
...
async function tagAsset(env, sb, asset, vocab) {
    ...
    if (!object)
        throw new Error(`missing from R2: ${asset.r2_key}`);
    ...
    await writeTags(sb, asset.id, result);
...
async function writeTags(sb, assetId, result) {
    const { data: tags } = await sb.from('tags').select('id, facet, value');
    const lookup = new Map((tags ?? []).map((t) => [`${t.facet}::${t.value}`, t.id]));
    const rows = [];
    for (const [facet, value] of Object.entries(result)) {
        ...
        for (const v of values) {
            ...
            if (id)
                rows.push({ asset_id: assetId, tag_id: id, source: 'ai' });
        }
    }
    ...
    if (rows.length)
        await sb.from('asset_tags').insert(rows);
}
...
async function threadContext(sb, assetId) {
    const { data } = await sb
