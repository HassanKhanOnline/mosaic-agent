...
export async function dedupeTick(env: Env): Promise<{ checked: number; merged: number; remaining: number }> {
  ...
  if (!batch?.length) return { checked: 0, merged: 0, remaining: 0 }
  ...
  for (const asset of batch) {
    ...
    const nearest = (nn as { asset_id: string; distance: number }[] | null)?.[0]
...
export async function mergePair(env: Env, sb: SupabaseClient, aId: string, bId: string) {
  ...
  const area = (x: any) => (x.width && x.height ? x.width * x.height : 0)
  const [keeper, dup] =
  ...
  if (occ?.length) {
    await sb.from('asset_occurrences').upsert(
      occ.map((o: any) => ({ ...o, asset_id: keeper.id })),
      { onConflict: 'asset_id,message_id', ignoreDuplicates: true },
    )
  }
  ...
  const { data: tags } = await sb
    ...
    .eq('asset_id', dup.id)
  if (tags?.length) {
    await sb.from('asset_tags').upsert(
      tags.map((t: any) => ({ ...t, asset_id: keeper.id })),
      { onConflict: 'asset_id,tag_id,source', ignoreDuplicates: true },
    )
  }

  const first =
    [keeper.first_seen_at, dup.first_seen_at].filter(Boolean).sort()[0] ?? keeper.first_seen_at
  const last =
    [keeper.last_seen_at, dup.last_seen_at].filter(Boolean).sort().at(-1) ?? keeper.last_seen_at
  ...
  if (dup.thumb_key) await env.BUCKET.delete(dup.thumb_key)
