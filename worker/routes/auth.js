...
auth.get('/google/callback', async (c) => {
    ...
    if (error)
        return c.redirect(`${c.env.APP_URL}/admin?error=${encodeURIComponent(error)}`);
    if (!code || !state)
        return c.text('missing code or state', 400);
    const userId = await checkState(state, c.env.TOKEN_KEY);
    if (!userId)
        return c.text('state expired or invalid', 400);
    ...
    if (!res.ok)
        return c.text(`token exchange failed: ${await res.text()}`, 502);
