...
export const canTag = (mime: string) => VISION_MIME.has(mime)
...
function schema(vocab: Vocabulary) {
  const list = (facet: string) => ({
    type: 'array',
...
export async function tagImage(
  ...
  context: { subject: string | null; threadText: string | null; filename: string | null },
  vocab: Vocabulary,
): Promise<TagResult> {
        ...
        content: [
          {
            ...
            source: {
              type: 'base64',
              media_type: image.mime as 'image/jpeg',
  ...
  if (!text || text.type !== 'text') throw new Error('no text block in tagging response')
  return JSON.parse(text.text) as TagResult
}
...
export function searchContent(
  result: TagResult,
  context: { subject: string | null; threadText: string | null },
): string {
