...
export interface JunkVerdict {
  reject: 'too_small' | 'bad_ratio' | null
}
...
export function classify(
  ...
  if (bytes < MIN_BYTES) return { reject: 'too_small' }
  if (dims) {
    if (dims.width < MIN_EDGE || dims.height < MIN_EDGE) return { reject: 'too_small' }
    ...
    if (ratio > MAX_RATIO) return { reject: 'bad_ratio' }
  }
  return { reject: null }
}
...
export function isBoilerplate(occurrences: number, firstSeen: string, lastSeen: string): boolean {
  if (occurrences < BOILERPLATE_OCCURRENCES) return false
  const spanDays = (Date.parse(lastSeen) - Date.parse(firstSeen)) / 86_400_000
  return spanDays > BOILERPLATE_SPAN_DAYS
}

export function objectKey(sha256: string, mime: string): string {
...
export async function store(
  ...
  bytes: Uint8Array,
  mime: string,
): Promise<{ key: string; thumb: string | null }> {
